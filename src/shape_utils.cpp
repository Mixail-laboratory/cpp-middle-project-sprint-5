...
std::optional<Shape> MakeCircle(const std::vector<double> &v) {
    return (v.size() == 3 ? std::optional{v} : std::nullopt).and_then([](const auto &vec) -> std::optional<Shape> {
        return (vec[2] > 0 ? std::optional{vec} : std::nullopt).and_then([](const auto &vec) -> std::optional<Shape> {
...
std::optional<Shape> MakeLine(const std::vector<double> &v) {
    return std::optional(v.size() == 4 ? std::optional{v} : std::nullopt)
        .and_then([](const auto &vec) -> std::optional<Shape> { return Line{{vec[0], vec[1]}, {vec[2], vec[3]}}; });
};
...
std::optional<Shape> MakeTriangle(const std::vector<double> &v) {
    return std::optional(v.size() == 6 ? std::optional{v} : std::nullopt)
        .and_then([](const auto &vec) -> std::optional<Shape> {
...
std::optional<Shape> MakeRectangle(const std::vector<double> &v) {
    return (v.size() != 4 ? std::optional{v} : std::nullopt).and_then([](const auto &vec) -> std::optional<Shape> {
        return (vec[2] <= 0 || vec[3] <= 0 ? std::optional{vec} : std::nullopt)
            .and_then(
                [](const auto &vec) -> std::optional<Shape> { return Rectangle{{vec[0], vec[1]}, vec[2], vec[3]}; });
    });
}
...
std::optional<Shape> MakePolygon(const std::vector<double> &v) {
    return (v.size() == 4 ? std::optional{v} : std::nullopt).and_then([](const auto &vec) -> std::optional<Shape> {
        return (vec[2] > 0 ? std::optional{vec} : std::nullopt).and_then([](const auto &vec) -> std::optional<Shape> {
            ...
            return (sides == vec[3] && sides >= 3 ? std::optional{sides} : std::nullopt).transform([&vec](int s) {
                return RegularPolygon{{vec[0], vec[1]}, vec[2], s};
            });
        });
    });
}
...
std::vector<std::pair<Shape, Shape>> FindAllCollisions(std::span<const Shape> shapes) {
    ...
    collisions.reserve(shapes.size() * (shapes.size() - 1) / 2);

    auto indices = std::views::iota(0u, shapes.size());

    auto pairs = indices | std::views::transform([&shapes](size_t i) {
    ...
    std::ranges::for_each(
        pairs | std::views::filter([&shapes](const auto &pair) {
            BoundingBox bb1 = queries::GetBoundBox(shapes[pair.first]);
            BoundingBox bb2 = queries::GetBoundBox(shapes[pair.second]);
            return bb1.Overlaps(bb2);
        }),
        [&collisions, &shapes](const auto &pair) { collisions.emplace_back(shapes[pair.first], shapes[pair.second]); });
